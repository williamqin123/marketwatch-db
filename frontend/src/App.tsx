...
import { ActionFeedbackToastsContext, Feedback } from './context/ActionFeedbackToastsContext';
...
function App() {
  ...
  return (
    ...
    {(()=>{if (signInDialog?.dialogState.isOpen) {
      ...
      return (<div className='fixed inset-0 z-[100] flex items-center justify-center' style={{backgroundColor:'#0005'}}>
        <Card>
        <CardHeader>
...
export async function apiCall(activeUserContext, actionFeedbackToastsContext, {endpoint, method, params}: {endpoint: string, method: 'GET'|'POST'|'PATCH'|'PUT'|'DELETE', params?: object}, callback: Function, doShowFeedbackToast: boolean, {successFeedbackMessage, failureFeedbackMessage}: {successFeedbackMessage?: string, failureFeedbackMessage?: string}={}) {
  ...
  if (hasParams && ['POST', 'PUT', 'PATCH', 'DELETE'].includes(method)) {
    ...
    const strParams: Record<string, string> = Object.fromEntries(Object.entries(params).map(([k,v]) => [k.toString(), v.toString()]));
  ...
  try {
      ...
      if (responseJson.hasOwnProperty('sql')) {
        actionFeedbackToastsContext.push(new Feedback('SUCCESS', successFeedbackMessage, url, paramsStringified, responseStatusCode, JSON.stringify(Object.fromEntries(Object.entries(responseJson).filter(([k,v]) => k !== 'sql'))), responseJson.sql));
      } else {
        actionFeedbackToastsContext.push(new Feedback('SUCCESS', successFeedbackMessage, url, paramsStringified, responseStatusCode, responseText));
      }
  }
  catch (error) {

    actionFeedbackToastsContext.push(new Feedback('FAIL', failureFeedbackMessage, url, paramsStringified, responseStatusCode, responseText));
